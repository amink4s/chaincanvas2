...
const GamePrototype: React.FC = () => {
    ...
    const composeSpectatorCast = async () => {
        if (!serverState) return;

        const turns = serverState.turns;
        const latestTurn = turns.length > 0 ? turns[turns.length - 1] : null;
        const imageUrl = latestTurn?.image_url || serverState.game.seed_image_url;
        const prompt = latestTurn?.prompt_text || serverState.game.seed_prompt || '';

        const turnCount = serverState.game.current_turn;
        const maxTurns = serverState.game.max_turns;
        const text = `Check out today's ChainCanvas! ${turnCount}/${maxTurns} mutations complete. ⏱️\n${MINI_APP_URL}`;

        let embeds: [string] | undefined;
        if (imageUrl) {
            const shareUrl = `${MINI_APP_URL}api/share?img=${encodeURIComponent(imageUrl)}&prompt=${encodeURIComponent(prompt)}`;
            embeds = [shareUrl];
        } else {
            embeds = [MINI_APP_URL];
        }
        ...
        <div className="h-full flex flex-col max-w-md mx-auto bg-black border-x border-slate-800 relative shadow-2xl overflow-y-auto">
            ...
            <div className="bg-slate-900 border-b border-slate-800 flex">
                <button
                    ...
                    className={`flex-1 px-4 py-3 text-sm font-semibold transition-colors flex items-center justify-center gap-2 ${activeTab === 'current'
                ...
                <button
                    ...
                    className={`flex-1 px-4 py-3 text-sm font-semibold transition-colors flex items-center justify-center gap-2 ${activeTab === 'history'
                            ? 'bg-slate-800 text-white border-b-2 border-indigo-500'
                            : 'text-slate-400 hover:text-slate-200'
                        }`}
                >
                ...
                {activeTab === 'history' ? (
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <h3 className="text-xs text-slate-400 uppercase tracking-wider font-bold">Seed Image</h3>
                            <div className="relative rounded-xl overflow-hidden border-2 border-slate-700 shadow-lg bg-slate-900">
                                <img src={serverState.game.seed_image_url} alt="Seed" className="w-full aspect-square object-cover" />
                                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-4">
                                    <p className="text-white text-sm font-medium">
                                        Seed: "{serverState.game.seed_prompt || 'No prompt'}"
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* All Turns */}
                        {serverState.turns.length > 0 && (
                            <div className="space-y-2">
                                <h3 className="text-xs text-slate-400 uppercase tracking-wider font-bold">
                                    Mutations ({serverState.turns.length})
                                </h3>
                                <div className="space-y-4">
                                    {serverState.turns.map((turn: any, idx: number) => (
                                        <div key={turn.id || idx} className="relative rounded-xl overflow-hidden border-2 border-slate-700 shadow-lg bg-slate-900">
                                            <img src={turn.image_url} alt={`Turn ${turn.turn_number}`} className="w-full aspect-square object-cover" />
                                            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-4">
                                                <p className="text-white text-sm font-medium">
                                                    @{turn.editor_username || `fid:${turn.editor_fid}`}: "{turn.prompt_text}"
                                                </p>
                    ...
                    <>
                        ...
                        {isMyTurn ? (
                            ...
                            <div className="space-y-4">
                                <div className="flex flex-col items-center justify-center py-8 px-4 text-center space-y-3 bg-slate-900/50 rounded-xl border border-slate-800 border-dashed">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-slate-400 text-sm max-w-[280px] mx-auto mt-2">
                                            {serverState.game.next_editor_username ? (
                                                <>
                                                    Waiting for <span className="text-indigo-400 font-semibold">@{serverState.game.next_editor_username}</span>.
                                                </>
                                            ) : (
                                                'Waiting for the current editor to finish.'
                                            )}
                                        </p>
                                    </div>
                                </div>

                                {/* Share Button for Spectators */}
                                <button
                                    onClick={composeSpectatorCast}
                                    className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all"
                                >
